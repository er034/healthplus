<html>
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no ">
    <meta name="description" content="Onlineshop für Nahrungsergänzungsmittel">
    <!-- Beschreibung unserer Webseite -->
    <meta name="keywords" content="Nahrungsergänzung, Online Shop , Fitness , Sport">
    <!-- Suchbbegriffe für die Suchmaschine -->
    <meta name="author" content="Eileen Röhm, Marc Lunardi, Cedric Tippl">
    <!-- Authoren der Webseite -->
    <title>HEALTH PLUS - ENJOY THE DIFFERENCE</title>
    <link rel= "stylesheet" href="style_abmelden.css" type="text/css">
</head>
</section>
<header>
    <div class="container">
        <div id="logo">
            <a href="index_alt.php"><img src="bilder/logo.svg" alt="Bild" /></a>
        </div>
        <nav>
            <ul>
                <li><a href="uebersicht_alt.php">PRODUKTE</a></li>
                <li><a href="angebote_abnehmpaket.php">ANGEBOTE</a></li>
                <li><a href="blog.php">BLOG</a></li>
            </ul>
        </nav>
    </div>
</header>
<section id="abgemeldet">
<div>
<h1>BIS ZUM NÄCHTSEN MAL!</h1>
    <a href="index_alt.php">ZURÜCK ZUM SHOP</a>'
</section>
</html>